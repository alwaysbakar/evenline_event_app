...
class OrderState {
  const OrderState({this.status = OrderStatus.idle});
...
class OrderCubit extends Cubit<OrderState> {
  ...
  Future<void> place(Event event, int quantity) async {
    emit(const OrderState(status: OrderStatus.loading));
    try {
      await store.book(event, quantity);
      emit(const OrderState(status: OrderStatus.success));
    } catch (_) {
      emit(const OrderState(status: OrderStatus.failure));
    }
  }
}
